...
ChildAlias::ChildAlias (Component* targetChild) :   target (targetChild), interest(false), userAdjusting(false)
{	
...
void ChildAlias::paint (Graphics& g)
{
	...
	g.fillAll ();
	g.setColour (c);
	g.drawRect (0,0,getWidth(),getHeight(),0);
}
...
ComponentLayoutEditor::~ComponentLayoutEditor ()
{
	//if (frames.size() > 0) frames.clear(true);
	...
	//if (nonMovableComponents.size() > 0) nonMovableComponents.clear(true);		// clear nonmovable array
}
...
{
	jassert (targetComp);
	//jassert (targetComp->getParentComponent() == getParentComponent()); // TODO: if left uncommented, it breaks code... look into this!
...
void ComponentLayoutEditor::updateFrames ()
{
		...
		//if (target && !target->hasBeenDeleted ())
	{
		...
		for (int i=1; i<n; i++) //JHOCHENBAUM Hack for now to not include the transport bar. Fix in actual GUI and pass different component here instead of master parent
		{
...
void ComponentLayoutEditor::addNonMovableComponent(Component* nmComp)
{
	nonMovableComponents.add(nmComp);
}
